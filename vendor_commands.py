...
def decode_version(raw):
    ...
    The vendor parser never reads these records, so the encoding comes from
    their firmware-version decoder. Which record is which was settled by
    upgrading a real device from 2.39 to 2.46: records 45 and 46 both tracked
    the change and are firmware; record 47 held at 1.01 across the upgrade and
    is something else -- hardware or bootloader revision.
    """
